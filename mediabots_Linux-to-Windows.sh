...
if [ $link1_status = "200" ] ; then 
	sudo wget -O /mediabots/WS2012R2.ISO https://archive.org/download/windows-server-2019-virtio_202106/Windows_Server_2019_Virtio.iso
elif [ $link2_status = "200" -o $link2_status = "301" -o $link2_status = "302" ] ; then 
	sudo wget -P /mediabots https://archive.org/download/windows-server-2019-virtio_202106/Windows_Server_2019_Virtio.iso
else
...
$qemupath -net nic,model=virtio ,hostfwd=tcp::3389-:3389 -show-cursor $custom_param_ram -localtime -enable-kvm -cpu host,hv_relaxed,hv_spinlocks=0x1fff,hv_vapic,hv_time,+nx -M pc -smp cores=$cpus -vga std -machine type=pc,accel=kvm -usb -device usb-tablet -k en-us -drive file=$custom_param_disk,index=0,media=disk -drive file=$custom_param_sw,index=1,media=cdrom $other_drives -boot c -vnc :9 &
pid2=$(echo $! | head -1)
disown -h $pid2
